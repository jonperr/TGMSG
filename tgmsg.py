...
def show_menu_banner():
    """Exibe o banner do menu principal no estilo TGMSG"""
    print("")
    print("███╗   ███╗███████╗███╗   ██╗██╗   ██╗")
    print("████╗ ████║██╔════╝████╗  ██║██║   ██║")
    print("██╔████╔██║█████╗  ██╔██╗ ██║██║   ██║")
...
def show_config_banner():
    """Exibe o banner de configurações no estilo TGMSG"""
    print("")
    print(" ██████╗ ██████╗ ███╗   ██╗███████╗")
    print("██╔════╝██╔═══██╗████╗  ██║██╔════╝")
    print("██║     ██║   ██║██╔██╗ ██║█████╗  ")
    ...
    print(" ╚═════╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝     ")
    print("")
    print("⚙️ Configurações do TGMSG")
    print("─" * 50)

def show_export_banner():
    """Exibe o banner de exportação no estilo TGMSG"""
    print("")
    print("████████╗ ██████╗ ███╗   ███╗███████╗ ██████╗ ")
    print("╚══██╔══╝██╔════╝ ████╗ ████║██╔════╝██╔════╝ ")
    print("   ██║   ██║  ███╗██╔████╔██║███████╗██║  ███╗")
...
def check_login_abuse(config):
    ...
    # Se não há registro anterior, permite login
    ...
    # Calcula diferença desde a última tentativa
    last_attempt = datetime.fromisoformat(config["last_login_attempt"])
    time_diff = (now - last_attempt).total_seconds() / 3600  # em horas
    
    # Se passou mais de 24 horas, reseta o contador
    if time_diff > 24:
        config["login_attempts"] = 1
        config["last_login_attempt"] = now.isoformat()
        save_config(config)
    ...
    # Atualiza contador e data da última tentativa
    config["login_attempts"] += 1
    config["last_login_attempt"] = now.isoformat()
    save_config(config)
    
    # Se excedeu 3 tentativas em 24 horas, alerta sobre possível banimento
...
async def setup_client():
    ...
    # Verifica abuso de login apenas quando realmente for fazer login
    ...
    if api_id.lower() in ['menu', 'voltar', 'sair']:
        return None
        
    api_hash = input("🔒 Digite seu API Hash: ").strip()
    if api_hash.lower() in ['menu', 'voltar', 'sair']:
        return None
...
async def export_chat(client, config):
    try:
        ...
        if group_link.lower() in ['menu', 'voltar', 'sair']:
        ...
        try:
            ...
            if has_topics_input in ['menu', 'voltar', 'sair']:
                return None, None, None
        ...
        if has_topics:
            ...
            if export_topic in ['menu', 'voltar', 'sair']:
                return None, None, None
            if export_topic == 's':
                topic_link = input("🔗 Cole o link do tópico: ").strip()
                if topic_link.lower() in ['menu', 'voltar', 'sair']:
                    return None, None, None
                ...
                if topic_match:
                    ...
                    try:
                        ...
                        if topic_id_input.lower() in ['menu', 'voltar', 'sair']:
                            return None, None, None
        ...
        # Usa o limite de mensagens das configurações ou pergunta
        limit = config["max_messages"]
        if not limit:
            try:
                limit_input = input("\n🔢 Quantidade máxima de mensagens a exportar (deixe em branco pra todas): ").strip()
                if limit_input.lower() in ['menu', 'voltar', 'sair']:
                    return None, None, None
        ...
        if confirm in ['menu', 'voltar', 'sair']:
            return None, None, None
        if confirm != 's':
...
async def collect_messages(client, group_entity, topic_id, limit, config):
    ...
    try:
        ...
        async for msg in client.iter_messages(group_entity, **kwargs):
            ...
            # Adiciona delay aleatório para prevenir banimento
...
async def config_menu(config):
    ...
    while True:
        ...
        print("↩️  Digite 'voltar' para retornar ao menu")
        print("")
        
        option = input("💡 Escolha uma opção: ").strip().lower()
        
        if option in ['voltar', 'menu', 'sair']:
            return config
...
async def main_menu(client):
    ...
    while True:
        ...
        print("   4. 🚪 Sair")
        print("")
        print("💡 Dica: Digite 'menu' a qualquer momento para voltar")
        print("")
        ...
        if option == '1':
            while True:
                ...
                # Nova pergunta após conclusão (como no código antigo)
                outro_grupo = input("\n🔄 Deseja exportar outro grupo? (s/n): ").strip().lower()
                if outro_grupo != 's':
                    # Opções quando não quer exportar outro grupo
                    ...
                    while True:
                        ...
                        if opcao == 'menu':
                            ...
                            print("\n👋 Tchau! Até a próxima 👋")
                            return False  # Indica que não é logout, apenas sair
                        else:
                            print("❌ Opção inválida. Tente novamente.")
            ...
            if await logout_menu(client, config):
                return True  # Indica que foi feito logout
                
        elif option == '4' or option == 'sair':
            print("\n👋 Tchau! Até a próxima 👋")
            return False  # Indica que não é logout, apenas sair
...
async def main():
    ...
    try:
        while True:
            ...
            if should_logout:
                # Usuário fez logout, então reiniciamos o processo de login
                continue
            else:
                # Usuário escolheu sair, então encerramos o programa
                break
